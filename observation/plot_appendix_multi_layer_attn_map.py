...
def parse_args():
    ...
    parser.add_argument("--layer_ids", nargs='+', type=int, default=[10, 14],
                       help="要分析的层数列表 (默认: [10, 14])")
    ...
    parser.add_argument("--output_dir", "-o", type=str, default="/home/zmw/idea/context_compression/ReasoningPathCompression/attn_heat_map_step/llama3",
                       help="输出的热力图文件夹")
    parser.add_argument("--figure_size", nargs=2, type=int, default=[24, 10],
                       help="图像大小 [宽, 高] (默认: [24, 10])")
...
def main():
    ...
    # 计算子图布局：如果层数<=4，使用1行；如果层数>4，使用2行
    num_layers = len(args.layer_ids)
    if num_layers <= 4:
        nrows, ncols = 1, num_layers
    else:
        nrows, ncols = 2, 4
    
    # 创建子图
    fig, axes = plt.subplots(nrows, ncols, figsize=tuple(args.figure_size))
    # 如果只有一个子图，确保axes是数组
    if num_layers == 1:
        axes = np.array([axes])
    else:
    ...
    for idx, layer_id in enumerate(args.layer_ids):
        ...
        ax.set_title(f'Layer {layer_id} Head {idx % 8}', fontsize=18)
    ...
    layer_ids_str = '_'.join(map(str, args.layer_ids))
    output_filename = f'{args.output_dir}/multi_layer_comparison_layers_{layer_ids_str}.png'
    plt.savefig(output_filename, bbox_inches='tight', dpi=300)
    print(f"多层对比热力图已保存至: {output_filename}")
    plt.close()
