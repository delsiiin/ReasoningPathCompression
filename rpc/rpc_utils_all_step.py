...
class RPCCluster():
    ...
    def compress_kv(self, origin_key_states, origin_value_states, row_sum_accu, col_sum_accu, num_key_value_groups, compressed_len, divisor):

        origin_col_sum_accu = col_sum_accu
        origin_row_sum_accu = row_sum_accu

        if (divisor is None) and (compressed_len is None):
            divisor = torch.arange(col_sum_accu.shape[-1], dtype=torch.int, device=col_sum_accu.device)
            divisor = col_sum_accu.shape[-1] - divisor  # 变成 [seq_len, seq_len-1, ..., 1]
            divisor = divisor.expand(col_sum_accu.shape[0], col_sum_accu.shape[1], -1)
        elif (divisor is not None) and (compressed_len is not None):
            offset = col_sum_accu.shape[-1] - compressed_len - self.R
            divisor = torch.cat([divisor, torch.zeros([col_sum_accu.shape[0], col_sum_accu.shape[1], offset], dtype=torch.int, device=col_sum_accu.device)], dim=-1)
            new_divisor = torch.arange(offset, dtype=torch.int, device=col_sum_accu.device)
            new_divisor = offset - new_divisor
            new_divisor = new_divisor.expand(col_sum_accu.shape[0], col_sum_accu.shape[1], -1)
            divisor[..., :-offset] += offset
            divisor[..., -offset:] += new_divisor

        
        row_sum_accu = row_sum_accu[..., :-self.R]
        col_sum_accu = col_sum_accu[..., :-self.R]

        col_sum_accu = col_sum_accu / divisor[..., :-self.R]
        ...
        divisor = torch.cat([divisor.gather(dim = 2, index = sum_indices), divisor[..., -self.R:]], dim=-1)
        ...
        return key_states, value_states, col_sum_accu, row_sum_accu, divisor, indices.shape[-2]
