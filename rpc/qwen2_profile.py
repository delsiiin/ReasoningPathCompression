...
from transformers.models.qwen2.modeling_qwen2 import repeat_kv, apply_rotary_pos_emb, rotate_half
...
def Qwen2Attention_grad_analysis_forward(
    self,
...
def Qwen2Model_use_attention_matrix_grad_log(self):
    """
    Set the model instance to use flash attention instead of llama attention
    """
    self.attention_matrix_log = defaultdict(list)
    for layer in self.layers:
        layer.self_attn.forward = MethodType(Qwen2Attention_grad_analysis_forward, layer.self_attn)
    self.get_attention_matrix_log = MethodType(Qwen2Model_get_attention_matrix_log, self)